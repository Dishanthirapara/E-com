        ...
        if user.is_authenticated:
            ...
            if re.match(r'^\d{10}$', user_mobile_no):
                if not user_firstname == "":
                    buyer_user.cart_buyer.first_name = user_firstname
                if not user_lastname == "":
                    buyer_user.cart_buyer.last_name = user_lastname
...
def user_insert_address(request):
    ...
    try:
        ...
        if user.is_authenticated:
            ...
            member = BuyerCheckout_details(checkout_street_address=checkout_street_address,
                                           checkout_apartment_address=checkout_apartment_address,
                                           checkout_pincode=checkout_pincode, checkout_city=checkout_city,
                                           checkout_ord_rec_mobile_no=checkout_order_receiver_mobile_no,
                                           checkout_select_state=checkout_select_state,
                                           checkout_ord_rec_name=checkout_order_receiver_name,
                                           checkout_buyer=buyer_user)
...
def user_view_address(request):
    try:
        ...
        if user.is_authenticated:
            ...
            member = BuyerCheckout_details.objects.filter(checkout_buyer=buyer_user)
...
def user_update_address(request):
    ...
    try:
        ...
        if user.is_authenticated:
            ...
            member = BuyerCheckout_details.objects.get(address=checkout_address, checkout_buyer=buyer_user)

            if not checkout_street_address == '':
                member.checkout_street_address = checkout_street_address
            if not checkout_apartment_address == '':
                member.checkout_apartment_address = checkout_apartment_address
            if not checkout_pincode == '':
                member.checkout_pincode = checkout_pincode
            if not checkout_city == '':
                member.checkout_city = checkout_city
            if not checkout_select_state == '':
                member.checkout_select_state = checkout_select_state
            if not checkout_order_receiver_name == '':
                member.checkout_ord_rec_name = checkout_order_receiver_name
            if not checkout_order_receiver_mobile_no == '':
                member.checkout_ord_rec_mobile_no = checkout_order_receiver_mobile_no
            member.save()
            serializer = BuyerAddressSerializer(member)
...
def user_insert_cart(request):
    ...
    try:
        ...
        if user.is_authenticated:
            ...
            member = BuyerCart(cart_qty=1, cart_product_color=cart_product_color, cart_product_size=cart_product_size,
                               cart_total=product_value.product_price,
                               cart_buyer=buyer_user, cart_product=product_value)
            member.save()
...
def user_view_cart(request):
    try:
        ...
        if user.is_authenticated:
            ...
            member = BuyerCart.objects.filter(cart_buyer=buyer_user)
            if member.exists():
...
def user_update_cart(request):
    ...
    try:
        ...
        if user.is_authenticated:
            ...
            member = BuyerCart.objects.get(Cart=user_cart, cart_buyer=buyer_user)
            # Calculate total based on quantity
            total = int(cart_qty) * member.cart_total

            if not member == "":
                if not cart_qty == '':
                    member.cart_qty = cart_qty
                if not cart_product_color == '':
                    member.cart_product_color = cart_product_color
                if not cart_product_size == '':
                    member.cart_product_size = cart_product_size

                member.cart_total = total
                member.save()
                serializer = BuyerCartSerializer(member)
...
def user_insert_buynow(request):
    ...
    try:
        ...
        if user.is_authenticated:
            ...
            member = BuyerPurchase(purchase_qty=qty, purchase_total=total, purchase_buyer=buyer_user,
                                   purchase_product=product_data, purchase_checkout=checkout_data)
            member.save()
    ...
    try:
        ...
        if user.is_authenticated:
            ...
            member = BuyerPurchase(
                purchase_qty=cart.cart_qty, purchase_total=cart.cart_total, purchase_buyer=buyer_user,
                purchase_product=cart.cart_product, purchase_cart=cart, purchase_checkout=checkout
            )
            member.save()
            # cart.delete()
            serializer = BuyercartPurchaseSerializer(member)
...
def create_payment_intent(request):
    # Extract data from the request
    user_purchase = request.data.get('purchase_data')
    try:
        ...
        if user.is_authenticated:
            ...
            intent = stripe.PaymentIntent.create(
                amount=details.purchase_total,
                currency='inr',
            )

            # Create a payment record in BuyerPayment
            payment = BuyerPayment.objects.create(
                payment_amount=details.purchase_total,
                payment_currency='INR',
                payment_status='Success',
                payment_intent_id=intent.id,
                payment_details=details,
                payment_buyer=buyer_user,
                payment_order_key=order_id
            )
        ...
        return Response({"error": "Invalid purchase id. Please select a valid purchase id."}, status=400)
...
def user_return_order(request):
    ...
    try:
        ...
        if user.is_authenticated:
            ...
            buyer_payment_instance = BuyerPayment.objects.get(payment_order_key=accept_instance.order)

            if not accept_instance == "" or buyer_payment_instance == "":

                # Create a Return instance
                return_instance = BuyerReturn(
                    return_buyer=buyer_user,
                    return_order=buyer_payment_instance,
                    returns=order_id,
...
def user_view_return(request):
    try:
        ...
        if user.is_authenticated:
            ...
            member = BuyerReturn.objects.filter(return_buyer=buyer_user)
            print(member)
            if not member == "":
...
def user_logout(request):
    try:
        user = request.user
        buyer_user = BuyerRegistration.objects.filter(buyer=user).first()
        print(buyer_user)

        if buyer_user:
            refresh_token = RefreshToken.for_user(user)  # Use the Django user instance here
            print(refresh_token)

            try:
                token = RefreshToken(refresh_token)
                print(token)
                # You might not need to manually create a RefreshToken instance here

                # Assuming you want to blacklist the token, uncomment the following line:
