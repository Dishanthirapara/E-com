...
def product_update(request, update_inventory_catalog, product_images, product_SKU, product_name, product_branding,
    ...
    if not product_color == '':
        update_inventory_catalog.cart_product_color = product_color
...
def rating_product(product_rating, products):
    ...
    data = {
        ...
        'product_id': products.cart_product,
...
def order_data(view_all_order):
    view_order = [{'order_id': order.return_order,
                   'image': order.payment.payment_details.cart_product.product_images,
                   'name': order.payment.payment_details.cart_product.product_name,
                   'product_sku': order.payment.payment_details.cart_product.product_SKU,
                   'company_id': order.company,
                   'quantity': order.cart_qty,
                   'size': order.cart_product_size,
...
def pending_order(request):
    try:
        ...
        if user:
            ...
            order_list = [order.return_order for order in buyer_all_order]
            for order in buyer_order:
                if order.payment_order_key not in order_list:
                    ...
                    orders = Order(
                        payment=order,
                        product=order.payment_details.cart_product,
                        buyer=order.cart_buyer,
                        qty=order.payment_details.cart_qty,
                        product_color=order.payment_details.cart_product.cart_product_color,
                        product_size=order.payment_details.cart_product.cart_product_size,
                        total=order.payment_amount,
                        order=order.payment_order_key,
                        company=company,
...
def accept_data(accept):
    accept_order = Accept(
        order=accept,
        product=accept.cart_product,
        buyer=accept.cart_buyer,
        qty=accept.cart_qty,
        product_color=accept.cart_product_color,
        product_size=accept.cart_product_size,
        total=accept.cart_total
    )
    accept_order.save()
    accept.cart_status = False
    accept.save()
    product = Product.objects.get(product_key=accept.cart_product.product_key)
    product.product_quantity = product.product_quantity - accept.cart_qty
    product.save()
    return accept
...
def ready_to_ship(request):
    try:
        ...
        if user:
            ...
            accept_orders_data = [{
                'order_id': order.return_order.return_order,
                'image': order.cart_product.product_images,
                'name': order.cart_product.product_name,
                'product_SKU': order.cart_product.product_SKU,
                'company_id': order.return_order.company,
                'quantity': order.cart_qty,
                'size': order.cart_product.cart_product_size,
                'dispatch_date': f"{order.return_order.dispatch_date.day} {order.return_order.dispatch_date.strftime('%b')}'{order.return_order.dispatch_date.strftime('%y')}"
...
def label_data(label, seller_user, purchase_order_no, invoice_no, invoice_date):
    ...
    if label.return_order.cart_product.product_category in ['BOOKS', 'BEAUTY', 'FURNITURE', 'GARDEN']:
        tax = label.cart_total * gst1 / 100
    elif label.return_order.cart_product.product_category in ['CLOTHE', 'MEN ACCESSORIES', 'WOMEN ACCESSORIES']:
        tax = label.cart_total * gst2 / 100
    elif label.return_order.cart_product.product_category == 'ELECTRONICS':
        tax = label.cart_total * gst3 / 100
    total = tax + label.cart_total
    data = {
        'customer_name': label.cart_buyer.cart_buyer.first_name + ' ' + label.cart_buyer.cart_buyer.last_name,
        'customer_address': label.cart_buyer.user_address,
        'seller_name': seller_user.register_user.first_name + seller_user.register_user.last_name,
        'seller_address': seller_user.address,
        'product_details': {'product_SKU': label.cart_product.product_SKU, 'size': label.cart_product.cart_product_size,
                            'qty': label.cart_qty,
                            'color': label.cart_product_color, 'order_no': label.return_order.return_order},
        'tax_invoice': {'ship_to': (
            label.return_order.payment.payment_details.purchase_checkout.checkout_ord_rec_name, label.return_order.payment.payment_details.purchase_checkout.checkout_street_address,
            label.return_order.payment.payment_details.purchase_checkout.checkout_apartment_address, label.return_order.payment.payment_details.purchase_checkout.checkout_pincode),
            'sold_by': (
                seller_user.register_user.first_name + " " + seller_user.register_user.last_name, seller_user.address),
            'purchase_order_no': purchase_order_no, 'invoice_no': invoice_no, 'order_date': label.return_order.order_date,
            'invoice_date': invoice_date},
        'description': label.cart_product.product_name, 'qty': label.cart_qty, 'gross_amount': total,
        'taxable_value': label.cart_total, 'taxes': f'GST RS.{tax}', 'total': total
    }
    return data
...
def cancel_data(cancel):
    order_cancel = Cancel(
        order=cancel,
        product=cancel.cart_product,
        buyer=cancel.cart_buyer,
        qty=cancel.cart_qty,
        product_color=cancel.cart_product_color,
        product_size=cancel.cart_product_size,
        total=cancel.cart_total
    )
    order_cancel.save()
    cancel.cart_status = False
    cancel.save()
    return cancel
...
def pricing_data(product, seller_user):
    ...
    for data in product:
        ...
        order_comprehension = [
            (total := total + j.cart_total, total_order := total_order + j.cart_qty)
            for j in orders
        ]
        data = {
            'product_details': {'image': data.product_images, 'name': data.product_name,
                                'product_SKU': data.product_SKU,
                                'size': data.cart_product_size},
...
def return_data(return_order):
    view_returns = [{
        'images': order.return_order.payment_details.cart_product.product_images,
        'name': order.return_order.payment_details.cart_product.product_name,
        'product_SKU': order.return_order.payment_details.cart_product.product_SKU,
        'category': order.return_order.payment_details.cart_product.product_category,
        'qty': order.return_order.payment_details.cart_qty,
        'size': order.return_order.payment_details.cart_product.cart_product_size,
        'order_id': order.return_order.payment_order_key,
...
def return_overview(request):
    try:
        ...
        if user:
            ...
            for i in product:
                return_order = BuyerReturn.objects.filter(order__cart__product__product_seller=seller_user,
                                                          order__cart__product__product=i.cart_product)
                order = Accept.objects.filter(product__product=i.cart_product, product__product_seller=seller_user)

            for data in product:
                return_order = BuyerReturn.objects.filter(order__details__product__product_seller=seller_user,
                                                          order__details__product__product=data.cart_product)
                order = Accept.objects.filter(product__product=data.cart_product, product__product_seller=seller_user)

                total, total_order = 0, 0
                for k in order:
                    total_order += k.cart_qty
